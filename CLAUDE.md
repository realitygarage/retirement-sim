...
- SS is **calendar-pegged like Medicare**, not year-index-relative: each spouse has independent `ssStartYear`/`ssStartMonth` (`ssBrendaStartYear`/`ssBrendaStartMonth` for Brenda) compared the same absolute-month way in both engines. `ssClaimAge(startYear, startMonth, birthYear, birthMonth)` (`engine.js`) is the one shared definition of "claiming age" — used by Bob's SS $-amount formula (continuous Early→FRA interpolation, generalized from the old stepped 65/66/67 toggle) and by the Simulator sidebar's derived age read-out. Brenda's SS dollar amount stays flat (`BASE.brendaSsFRA`) regardless of her claim date — no early/delayed formula exists for her (a deliberate scope decision, not an oversight; revisit only with new real numbers for her early-claim amount).
- `buildScenario`'s annual engine can't just gate SS on/off per row like the old year-only check did — a spouse's claim can start mid-row (mid-partial-first-year, or mid-full-year for a scenario override). `yourSs`/`brendaSs` are therefore **period dollar totals**, summed month-by-month the same way `healthAnnual` already was (reusing `monthOffset`, not parallel logic), then divided back down to a displayed `$/mo` rate at every read site (`row.push`, `estimateTax`'s $/mo parameter) following the same "`*12`-in, `/monthsThisYear`-out" convention v4.3.0 established. If you add a new age-triggered $ field to the annual engine, follow this same period-total-then-divide pattern rather than a flat per-row constant.
